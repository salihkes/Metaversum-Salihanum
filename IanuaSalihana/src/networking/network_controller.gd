...
func _try_web_sso_login():
    "Attempt SSO login when running in web browser.\n\t\n\tThis function:\n\t1. Calls JavaScript to fetch an auth token from the website\n\t2. If successful, sends the token to the game server for authentication\n\t\n\tRequirements:\n\t- The Godot game must be served from the same origin as the website\n\t- User must be logged in on the website (has session cookie)\n\t"
...
func _setup_sso_callback():
    "Set up JavaScript callback for SSO token retrieval"
...
func _check_sso_result():
    "Check if SSO token was retrieved from JavaScript"
...
func _on_sso_token_received(args):
    "Callback when SSO token is received from JavaScript"
    if args.size() > 0:
        var result = args[0]
        print("[SSO] Callback received: ", result)

func send_token_login(token: String):
    "Send token-based login request (for Web SSO)"
    if not _connected:
